...
#ifdef TARGET_OS2
static int midi_map[SNDRV_CARDS] = {REPEAT_SNDRV(0)};
static int amidi_map[SNDRV_CARDS] = {REPEAT_SNDRV(1)};
#else
                            ...
                            int mode, snd_rawmidi_file_t * rfile)
{
    ...
    printk("snd_rawmidi_kernel_open. mode: %i\n", mode);
    if (rfile)
