...
static int external_amp[SNDRV_CARDS] = {REPEAT_SNDRV(1)};
static int amp_gpio[SNDRV_CARDS] = {REPEAT_SNDRV(-1)};
...
snd_m3_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
    ...
    if (status & ASSP_INT_PENDING) {
        u8 ctl;
        ctl = inb(chip->iobase + ASSP_CONTROL_B);
    ...
    *chip_ret = NULL;
    ...
    for (quirk = m3_quirk_list; quirk->vendor; quirk++) {
        if (pci->subsystem_vendor == quirk->vendor &&
            ...
            snd_printk("maestro3: enabled hack for '%s'\n", quirk->name);
            chip->quirk = quirk;
            break;
        }
    }
...
#ifdef TARGET_OS2
    if (((pci->_class >> 8) & 0xffff) != PCI_CLASS_MULTIMEDIA_AUDIO)
#else
...
#ifdef DEBUG
    dprintf(("m3_probe: create passed"));
#endif
