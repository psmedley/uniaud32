...
static int snd_pcm_hw_refine_old_user(snd_pcm_substream_t * substream, struct snd_pcm_hw_params_old * _oparams);
static int snd_pcm_hw_params_old_user(snd_pcm_substream_t * substream, struct snd_pcm_hw_params_old * _oparams);
                      ...
                      snd_pcm_hw_params_t *params)
{
...
#ifdef DEBUG
        dprintf(("snd_pcm_hw_refine error. rules > 32"));
#endif
    ...
    for (k = SNDRV_PCM_HW_PARAM_FIRST_INTERVAL; k <= SNDRV_PCM_HW_PARAM_LAST_INTERVAL; k++) {
        ...
        {
#ifdef DEBUG
            dprintf(("snd_interval_empty() error"));
#endif
            return -EINVAL;
        }
        if (!(params->rmask & (1 << k)))
    ...
    do {
        ...
        for (k = 0; k < constrs->rules_num; k++) {
            ...
            if (!doit)
                continue;
#ifdef RULES_DEBUG
            printk("Rule %d (r->var=%d) [%p]: ", k, r->var, r->func);
            if (r->var >= 0) {
            ...
            {
#ifdef DEBUG
                dprintf(("invalid changed = %d rule addr: %x, k = %d",changed,r->func,k));
#endif
                return changed;
            }
            stamp++;
...
static int snd_pcm_hw_refine_user(snd_pcm_substream_t * substream, snd_pcm_hw_params_t * _params)
{
    ...
    int err;
#ifdef DEBUG
    //dprintf(("snd_pcm_hw_refine_user()"));
#endif
...
#ifdef DEBUG
    if (err)
        dprintf(("snd_pcm_hw_refine_user() rc = %d",err));
#endif
                             ...
                             snd_pcm_hw_params_t *params)
{
    ...
    if (err < 0)
    {
        printk("hw params refine error = %i\n",err);
        goto _error;
    }
    ...
    if (err < 0)
    {
        printk("hw params choose error = %i\n",err);
        goto _error;
    }
...
static struct action_ops snd_pcm_action_suspend = {
    ...
    /*.do_action =  */ snd_pcm_do_suspend, 0,
...
static struct action_ops snd_pcm_action_prepare = {
    ...
    /*.do_action =   */snd_pcm_do_prepare, 0,
...
static struct action_ops snd_pcm_action_drain_init = {
    ...
    /*.do_action = */ snd_pcm_do_drain_init, 0,
...
#ifdef TARGET_OS2
    file = (struct file *)substream->file;
#else
                             ...
                             snd_pcm_file_t **rpcm_file)
{
    ...
    printk("snd_pcm_open_file %X\n",rpcm_file);

    snd_assert(rpcm_file != NULL, return -EINVAL);
    *rpcm_file = NULL;

    pcm_file = (snd_pcm_file_t *)kzalloc(sizeof(*pcm_file), GFP_KERNEL);
    if (pcm_file == NULL) {
        return -ENOMEM;
    }

    if ((err = snd_pcm_open_substream(pcm, stream, &substream)) < 0) {
        printk("snd_pcm_open_file: error open substream %i\n",err);
        kfree(pcm_file);
        return err;
    }
...
int snd_pcm_open(struct inode *inode, struct file *file)
{
    ...
    wait_queue_t wait;
    //#ifdef DEBUG_PK
    printk("snd_pcm_open card: %i, dev: %i\n", cardnum, device);
    //#endif
    ...
    //        printk("snd_pcm_open: snd_card_file_add %i\n",err);
    if (err < 0)
        goto __error1;
...
static int snd_pcm_sync_ptr(snd_pcm_substream_t *substream, struct snd_pcm_sync_ptr *_sync_ptr)
{
    snd_pcm_runtime_t *runtime = substream->runtime;
                                 ...
                                 unsigned int cmd, void *arg)
{
    snd_assert(substream != NULL, return -ENXIO);
#ifdef DEBUG
    //        dprintf(("vladest: snd_pcm_common_ioctl1. cmd = %d",cmd));
#endif
    ...
    case SNDRV_PCM_IOCTL_SYNC_PTR:
        return snd_pcm_sync_ptr(substream, (struct snd_pcm_sync_ptr *) arg);
    case SNDRV_PCM_IOCTL_HW_REFINE_OLD:
        return snd_pcm_hw_refine_old_user(substream, (struct snd_pcm_hw_params_old *) arg);
    case SNDRV_PCM_IOCTL_HW_PARAMS_OLD:
        return snd_pcm_hw_params_old_user(substream, (struct snd_pcm_hw_params_old *) arg);
                                   ...
                                   unsigned int cmd, void *arg)
{
    ...
    //#ifdef DEBUG
    //        dprintf2(("vladest: snd_pcm_playback_ioctl1. cmd = %d",cmd));
    //#endif
    switch (cmd) {
...
#ifdef DEBUG
    //        dprintf2(("vladest: snd_pcm_capture_ioctl1. cmd = %d",cmd));
#endif
    switch (cmd) {
...
static ssize_t snd_pcm_write(struct file *file, const char *buf, size_t count, loff_t * offset)
{
...
#ifdef DEBUG_PK
    printk("snd_pcm_write. count = %i, offset = %i\n",count, offset);
#endif
    pcm_file = file->private_data;
    substream = pcm_file->substream;
    ...
    if (!frame_aligned(runtime, count)) {
#ifdef DEBUG_PK
        printk("snd_pcm_write. frame_aligned error. byte align = %i\n",runtime->byte_align);
#endif
        result = -EINVAL;
        goto end;
    }
...
#ifdef DEBUG_PK
    printk("snd_pcm_writev. count = %i, offset = %i\n",count, offset);
#endif
    pcm_file = file->private_data;
    substream = pcm_file->substream;
    snd_assert(substream != NULL, result = -ENXIO; goto end);
...
static void snd_pcm_hw_convert_from_old_params(snd_pcm_hw_params_t *params, struct snd_pcm_hw_params_old *oparams)
{
    unsigned int i;
...
static void snd_pcm_hw_convert_to_old_params(struct snd_pcm_hw_params_old *oparams, snd_pcm_hw_params_t *params)
{
    unsigned int i;
