...
inline int vortex_wtdma_getlinearpos(vortex_t *vortex, int wtdma) {
...
inline int vortex_adbdma_getlinearpos(vortex_t *vortex, int adbdma) {
    stream_t *dma = &vortex->dma_adb[adbdma];
    int temp;

    temp = hwread(vortex->mmio, VORTEX_ADBDMA_STAT + (adbdma << 2));
#ifdef DEBUG
//    dprintf(("adbdma: %x, temp %x, per virt %x, per bytes %x",adbdma,temp,dma->period_virt, dma->period_bytes));
#endif
...
#ifdef DEBUG
	dprintf(("Vortex: allocated bytes %d", params_buffer_bytes(hw_params)));
#endif
	if (err < 0) {
#ifdef DEBUG
            dprintf(("Vortex: pcm page alloc failed!"));
#endif
		return err;
        }
#ifdef DEBUG
	dprintf(("Vortex: periods %d, period_bytes %d, channels = %d", params_periods(hw_params),
...
#endif
#ifdef DEBUG
//        dprintf(("vortex: pointer = 0x%x", current_ptr));
#endif
...
//    snd_pcm_lib_preallocate_pci_pages_for_all(chip->pci_dev, pcm, 0x10000, 0x20000);
