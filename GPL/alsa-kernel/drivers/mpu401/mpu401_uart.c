...
static void snd_mpu401_uart_input_read(mpu401_t * mpu)
{
    ...
    while (max-- > 0) {
        if (snd_mpu401_input_avail(mpu)) {
            ...
            printk("mpu uart: %x\n",byte);
            //vladest                    if (test_bit(MPU401_MODE_BIT_INPUT_TRIGGER, &mpu->mode))
...
static snd_rawmidi_ops_t snd_mpu401_uart_output =
{
    ...
    snd_mpu401_uart_output_trigger,0
};
...
static snd_rawmidi_ops_t snd_mpu401_uart_input =
{
    ...
    snd_mpu401_uart_input_trigger,0
};
#else
static snd_rawmidi_ops_t snd_mpu401_uart_output =
{
                        ...
                        snd_rawmidi_t ** rrawmidi)
{
    ...
    printk("snd_mpu401_uart_new on irq: %i, port: %i\n",irq,port);
    if (rrawmidi)
        *rrawmidi = NULL;
    ...
    if (card->shortname[0])
        sprintf(rmidi->name, "%s MPU-401", card->shortname);
    else
