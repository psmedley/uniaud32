...
static int snd_timer_check_slave(struct snd_timer_instance *slave)
{
	...
	list_for_each_entry(timer, &snd_timer_list, device_list, struct snd_timer) {
		list_for_each_entry(master, &timer->open_list_head, open_list, struct snd_timer_instance) {
...
static int snd_timer_check_master(struct snd_timer_instance *master)
{
	...
	list_for_each_entry_safe(slave, tmp, &snd_timer_slave_list, open_list, struct snd_timer_instance) {
		err = check_matching_master_slave(master, slave);
		if (err < 0)
			break;
	}
				   ...
				   struct device **card_devp_to_put)
{
	...
	if (timer) {
		...
		list_for_each_entry_safe(slave, tmp, &timeri->slave_list_head,
					 open_list, struct snd_timer_instance) {
...
static void snd_timer_reschedule(struct snd_timer * timer, unsigned long ticks_left)
{
	...
	list_for_each_entry(ti, &timer->active_list_head, active_list, struct snd_timer_instance) {
...
// can't define this in interrupt.h due to use of typeof in original definition
#define from_tasklet(var, callback_tasklet, tasklet_fieldname)	\
	container_of(callback_tasklet, struct snd_timer, tasklet_fieldname)
...
void snd_timer_interrupt(struct snd_timer * timer, unsigned long ticks_left)
{
	...
	list_for_each_entry_safe(ti, tmp, &timer->active_list_head,
				 active_list, struct snd_timer_instance) {
		if (ti->flags & SNDRV_TIMER_IFLG_DEAD)
			continue;
		if (!(ti->flags & SNDRV_TIMER_IFLG_RUNNING))
			continue;
...
static int snd_timer_dev_disconnect(struct snd_device *device)
{
	...
	list_for_each_entry(ti, &timer->open_list_head, open_list, struct snd_timer_instance) {
		if (ti->disconnect)
			ti->disconnect(ti);
	}
	mutex_unlock(&register_mutex);
...
#define from_timer(var, callback_timer, timer_fieldname) \
	container_of(callback_timer, struct snd_timer_system_private, timer_fieldname)
...
#else
			tread32.event = tread->event;
			tread32.tstamp_sec = tread->tstamp_sec;
			tread32.tstamp_nsec = tread->tstamp_nsec;
			tread32.val = tread->val;
#endif
...
static void snd_timer_free_all(void)
{
	...
	list_for_each_entry_safe(timer, n, &snd_timer_list, device_list, struct snd_timer)
		snd_timer_free(timer);
}
