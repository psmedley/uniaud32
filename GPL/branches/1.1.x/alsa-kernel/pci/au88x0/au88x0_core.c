...
int  vortex_adbdma_bufshift(vortex_t *vortex, int adbdma) {
	stream_t *dma = &vortex->dma_adb[adbdma];
	...
	if (delta == 0)
		goto _exit;
#ifdef DEBUG
        dprintf(("delta: %x, page: %x, nr_per: %x, per r: %x, per v:  %x",delta, page, dma->nr_periods,dma->period_real,dma->period_virt));
#endif
...
int  vortex_wtdma_bufshift(vortex_t *vortex, int wtdma) {
	stream_t *dma = &vortex->dma_wt[wtdma];
	int page, p, delta, i;
...
void vortex_wtdma_chain(vortex_t *vortex, int wtdma, signed char sb1, signed char sb0) {
    stream_t *dma = &vortex->dma_wt[wtdma];
    int *cfg;
    int offset;
...
*/
int  vortex_adb_checkinout(vortex_t *vortex, u32 resmap[], int out, int restype) {
...
void vortex_codec_write(ac97_t *codec, unsigned short addr, unsigned short data){
	...
	unsigned long flags;
        ...
        unsigned int temp_addr, temp_data;
        int temp;

//        temp_addr = (unsigned int) addr;
//        temp_data = (unsigned int) data;
