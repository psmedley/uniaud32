...
static struct snd_seq_kinstr *snd_seq_instr_new(int add_len, int atomic)
{
	...
	instr = (struct snd_seq_kinstr *)kzalloc(sizeof(struct snd_seq_kinstr) + add_len, atomic ? GFP_ATOMIC : GFP_KERNEL);
	if (instr == NULL)
		return NULL;
...
struct snd_seq_kinstr_list *snd_seq_instr_list_new(void)
{
	...
	list = (struct snd_seq_kinstr_list *)kzalloc(sizeof(struct snd_seq_kinstr_list), GFP_KERNEL);
	if (list == NULL)
		return NULL;
		     ...
		     int atomic, int hop)
{
	...
	if (instr->type == SNDRV_SEQ_INSTR_ATYPE_DATA) {
		result = ops->put(ops->private_data,
				  instr,
				  (char __user *)ev->data.ext.ptr + sizeof(struct snd_seq_instr_header),
