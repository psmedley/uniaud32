...
#endif
#ifdef TARGET_OS2
#define __BASE_FILE__ "seq_clientmgr.c"
#endif
				  ...
				  int atomic, int hop)
{
	...
	if (atomic)
		...
		list_for_each_entry(subs, &grp->list_head, src_list, struct snd_seq_subscribers) {
		/* both ports ready? */
				...
				int atomic, int hop)
{
	...
	list_for_each_entry(port, &dest_client->ports_list_head, list, struct snd_seq_client_port) {
