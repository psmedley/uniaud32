...
struct snd_kcontrol *snd_ctl_new(struct snd_kcontrol * control, unsigned int access)
{
    ...
    kctl = kcalloc(1, sizeof(*kctl) + sizeof(struct snd_kcontrol_volatile) * control->count, GFP_KERNEL);
    if (kctl == NULL)
        return NULL;
    *kctl = *control;
 ...
 * Returns 0 if successful, or a negative error code on failure.
 */
static int snd_ctl_remove_unlocked_id(struct snd_ctl_file * file, struct snd_ctl_elem_id *id)
{
...
static int snd_ctl_elem_list(struct snd_card *card, struct snd_ctl_elem_list *_list)
{
			    ...
			    struct snd_ctl_elem_info *info, int replace)
{
	...
	ue = (struct user_element *)kzalloc(sizeof(struct user_element) + private_size, GFP_KERNEL);
	if (ue == NULL)
		return -ENOMEM;
	ue->info = *info;
...
static int snd_ctl_subscribe_events(struct snd_ctl_file *file, int *ptr)
{
    int subscribe;
                         ...
                         unsigned int cmd, unsigned long arg)
{
    ...
    case SNDRV_CTL_IOCTL_ELEM_LIST:
        return snd_ctl_elem_list(ctl->card, (struct snd_ctl_elem_list *) arg);
    ...
    case SNDRV_CTL_IOCTL_ELEM_UNLOCK:
        return snd_ctl_elem_unlock(ctl, (struct snd_ctl_elem_id *) arg);
    case SNDRV_CTL_IOCTL_ELEM_ADD:
        return snd_ctl_elem_add(ctl, (struct snd_ctl_elem_info *) arg, 0);
    case SNDRV_CTL_IOCTL_ELEM_REPLACE:
        return snd_ctl_elem_add(ctl, (struct snd_ctl_elem_info *) arg, 1);
    case SNDRV_CTL_IOCTL_ELEM_REMOVE:
        return snd_ctl_elem_remove(ctl, (struct snd_ctl_elem_id *) arg);
