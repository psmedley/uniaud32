...
static int total_bufsize[SNDRV_CARDS] = { REPEAT_SNDRV(1024) };
static int pcm_substreams_p[SNDRV_CARDS] = { REPEAT_SNDRV(4) };
static int pcm_substreams_c[SNDRV_CARDS] = { REPEAT_SNDRV(1) };
                                      ...
                                      const struct pci_device_id *pci_id)
{
    ...
    printk("snd_es1968_probe cp2\n");
    card = snd_card_new(index[dev], id[dev], THIS_MODULE, 0);
    if (!card)
        return -ENOMEM;

    printk("snd_es1968_probe cp3\n");
    ...
    if ((err = snd_card_register(card)) < 0) {
        ...
        printk("snd_es1968_probe cp11\n");

    pci_set_drvdata(pci, card);
        printk("snd_es1968_probe cp12\n");
