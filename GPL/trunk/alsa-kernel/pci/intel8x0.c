...
static int __devinit snd_intel8x0_mixer(intel8x0_t *chip, int ac97_clock, char *quirk_override)
{
    ac97_bus_t *pbus;
...
//    pci_write_config_byte(pci, 0x40, 0xff); // added by vladest
    ...
    /*
     есть два нюанса. Надо смотреть. Первое: pci_cfg_space 54h,
     два младших бита указывают состояние чипа. если там 11, то
     кроме конфигспейса больше ни хрена не работает.
     Туда надо записать 00.
     Второе: регистр PCICMD, это 16-битный регистр по смещению 04h
     в cfg_space. Бит 1 выставить в 1
     */
#if 1
