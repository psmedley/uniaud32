/* $Id: rmbase.h,v 1.1.1.1 2003/07/02 13:56:56 eleph Exp $ */
/*
 * OS/2 Resource Manager types and definitions
 *
...
typedef ULONG    RMHANDLE, *PRMHANDLE, NEAR *NPRMHANDLE;
...
}AHRESOURCE, NEAR *NPAHRESOURCE, FAR48 *LPAHRESOURCE;
...
typedef struct {
   ...
   UCHAR  Month;
   UCHAR  Day;
} DATESTAMP, *PDATESTAMP, NEAR *NPDATESTAMP;


/* Callback for Grant/Yield; Indicates CM resolves resource to handle */
typedef FARPTR16 PFNRMCB;


typedef struct {
   FARPTR16  DrvrName;
   FARPTR16  DrvrDescript;
   FARPTR16  VendorName;
   UCHAR     MajorVer;
   UCHAR     MinorVer;
   DATESTAMP Date;
...
} DRIVERSTRUCT, *PDRIVERSTRUCT, NEAR *NPDRIVERSTRUCT;
...
typedef struct {
   USHORT ADDHandle;
   USHORT UnitHandle;
}ADD_UNIT,  *PADD_UNIT, NEAR *NPADD_UNIT;
...
typedef struct {
   ...
   ULONG  VolSize;
   ULONG  VolID;
} DASD_VOL,  *PDASD_VOL, NEAR *NPDASD_VOL;
...
} DRIVERINFO,  *PDRIVERINFO, NEAR *NPDRIVERINFO;

#define DriverBase 0
#define DriverLoadable 1


typedef struct _ADJUNCT *PADJUNCT;
...
typedef struct{
   ...
   USHORT       HostBusType;
   USHORT       HostBusWidth;
   FARPTR16     pAdjunctList;
   ULONG        Reserved;             /* Logical Name addition? */
} ADAPTERSTRUCT,  *PADAPTERSTRUCT, NEAR *NPADAPTERSTRUCT;
...
typedef struct {
   FARPTR16 DevDescriptName;
   USHORT   DevFlags;
   USHORT   DevType;
   FARPTR16 pAdjunctList;
} DEVICESTRUCT,  *PDEVICESTRUCT, NEAR *NPDEVICESTRUCT;
...
typedef struct {
   ...
   }AHDEVICES,  *PAHDEVICES, NEAR *NPAHDEVICES;
...
} IORESOURCE,  *PIORESOURCE, NEAR *NPIORESOURCE;
...
#else
typedef USHORT (_cdecl  *PFNRMINTHANDLER)(VOID);
#endif
...
} IRQRESOURCE,  *PIRQRESOURCE, NEAR *NPIRQRESOURCE;
...
} MEMRESOURCE,  *PMEMRESOURCE, NEAR *NPMEMRESOURCE;
...
typedef struct {
   USHORT DMAChannel;
   USHORT DMAFlags;
} DMARESOURCE,  *PDMARESOURCE, NEAR *NPDMARESOURCE;
...
typedef struct {
   USHORT TMRChannel;
   USHORT TMRFlags;
} TMRRESOURCE,  *PTMRRESOURCE, NEAR *NPTMRRESOURCE;
...
typedef struct {
   ...
   ULONG  Reserved;
} RESOURCESTRUCT,  NEAR *NPRESOURCESTRUCT, FAR48 *LPRESOURCESTRUCT;
...
typedef struct {
   FARPTR16 LDevDescriptName;
   USHORT   LDevFlags;
   USHORT   LDevClass;
   HDEVICE  LDevHDevice;
   FARPTR16 pAdjunctList;
} LDEVSTRUCT,  *PLDEVSTRUCT, NEAR *NPLDEVSTRUCT;
...
typedef struct {
   FARPTR16 SysDescriptName;
   FARPTR16 pAdjunctList;
   USHORT   SysFlags;
   USHORT   Reserved;
} SYSNAMESTRUCT,  *PSYSNAMESTRUCT, NEAR *NPSYSNAMESTRUCT;
...
typedef struct {
   FARPTR16 DetectDescriptName;/* Ascii name       */
   USHORT   DetectFlags;       /* ???              */
   ...
   VENDID   VendorID;
   SERNUM   SerialNumber;
} DETECTEDSTRUCT,  *PDETECTEDSTRUCT, NEAR *NPDETECTEDSTRUCT;
...
} HANDLELIST,  NEAR *NPHANDLELIST, FAR48 *LPHANDLELIST;
...
} ADJHANDLELIST,  *PADJHANDLELIST, NEAR *NPADJHANDLELIST;
